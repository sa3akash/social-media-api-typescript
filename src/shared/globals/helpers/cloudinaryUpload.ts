...
const storage = new CloudinaryStorage({
  ...
  params: () => {
    return {
      ...
      // allowed_formats: ['jpg', 'jpeg', 'png', 'gif', 'webp'],
      quality: 'auto:good', // Set the desired quality level (e.g., auto:low, auto:good, auto:best)
      resource_type: 'auto' // Optional: Specify the resource type (image, video, raw, etc.)
    };
  }
});
...
export async function updateImage(publicId: string, newUrl: string) {
  try {
    const result = await cloudinary.v2.uploader.rename(publicId, newUrl, { overwrite: true });
    return result;
  } catch (error) {
    throw new BadRequestError('file update failed.');
  }
}
